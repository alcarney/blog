...
If the current buffer is associated with a project, show the name of the project, as returned by ``(project-name pr)``

.. tip::

   By default a VC backed project will report the name of the folder containing the project which for me can be confusing sometimes.
   I've started organising my projects in the following structure ``Projects/<owner>/<repo>/<branch>``, where each ``<branch>`` is a `git worktree <https://git-scm.com/docs/git-worktree>`__.
   This can lead to multiple projects showing as ``main`` or ``develop`` etc.

   However, this name can be overridden by setting ``project-vc-name`` in a ``.dir-locals.el`` file.
